...
import { Users, User, Search, ShieldAlert, ShieldCheck, Trash2, Loader2, Plus, Upload, Download } from "lucide-react";
...
async function deleteStudentAccount(userId, email) {
  return authenticatedFetch("/api/admin/delete-student", {
    method: "POST",
...
function AdminStudentsPage() {
  ...
  async function handleFileUpload(e) {
    ...
    try {
      ...
      const response = await fetch(buildApiUrl("/api/admin/bulk-upload-whitelist"), {
        method: "POST",
  ...
  const allEntries = (() => {
    ...
    students.forEach((stud) => {
      ...
      entriesMap.set(emailLower, {
        ...
        avatar_url: stud.avatar_url || null
      });
    });
              ...
              return (<TableRow key={ent.email} className="hover:bg-muted/30 transition-colors">
                <TableCell className="font-semibold text-foreground">
                  <div className="flex items-center gap-2.5">
                    {ent.avatar_url ? (
                      <div className="h-8 w-8 rounded-full border bg-background overflow-hidden shrink-0 flex items-center justify-center shadow-sm">
                        <img src={ent.avatar_url} alt={ent.name} className="h-full w-full object-cover" />
                      </div>
                    ) : (
                      <div className="h-8 w-8 rounded-full border bg-muted flex items-center justify-center shrink-0">

  ...
  head: () => ({
    meta: [
      { title: "Karta Connect — Student & Company Network" },
      {
        name: "description",
        content:
          "A platform by Karta Foundation connecting students, companies, and opportunities.",
      },
    ],
  }),
  component: Landing,
  ...
  return (
    <div className="absolute inset-0 overflow-hidden z-0">
      {bgImages.map((src, idx) => (
        <div
          key={src}
          className={`absolute inset-0 bg-cover bg-center transition-all duration-1000 ease-in-out ${idx === index ? "opacity-100 scale-100" : "opacity-0 scale-105"}`}
...
function QuoteRotator() {
  ...
  return (
    <div className="flex flex-col justify-center h-full p-8 text-white space-y-6">
      ...
      <div
        className={`transition-opacity duration-500 min-h-[140px] flex flex-col justify-between ${fade ? "opacity-100" : "opacity-0"}`}
      >
      ...
      <header className="relative z-20 w-full px-6 py-4">
        <div className="mx-auto flex max-w-6xl items-center justify-between">
          ...
          <Link to="/login">
            <Button
              variant="outline"
              className="border-white/20 text-white hover:bg-white/10 hover:text-white text-xs px-4 py-1.5 h-8"
            >
              Sign in
            </Button>
          </Link>
        ...
        <div className="w-full max-w-4xl bg-black/40 border border-white/10 rounded-3xl backdrop-blur-md overflow-hidden shadow-2xl flex flex-col md:flex-row min-h-[440px] animate-in fade-in duration-700">
          ...
          <div className="w-full md:w-[380px] p-8 flex flex-col justify-center space-y-6">
            <div className="bg-white rounded-2xl p-4 max-w-[180px] mx-auto shadow-md border border-white/10 flex items-center justify-center backdrop-blur-sm">
              <img
                src="/karta-logo.png"
                className="w-full h-auto object-contain max-h-[54px]"
                alt="Karta Initiative Logo"
              />
            </div>
            <div className="text-center space-y-2">
              ...
              <p className="text-xs text-zinc-400 max-w-xs mx-auto leading-relaxed">
                Access your Karta matching network profile, opportunities
                pipeline, and admin controls.
              </p>
            </div>
            <div className="pt-2">
              <Link to="/login" className="w-full">
                <Button
                  size="lg"
                  className="w-full font-bold shadow-lg hover:scale-102 transition-transform duration-200 flex items-center justify-center gap-2"
                >
          ...
          <div className="relative w-full overflow-hidden">
            ...
            <div className="animate-marquee flex gap-8 items-center py-1.5">
              {partners.map((partner, i) => (
                <div
                  key={i}
                  className="flex items-center justify-center bg-white/5 px-5 py-2.5 rounded-xl border border-white/10 shadow-sm min-w-[150px] h-[48px] shrink-0"
                >
                  {partner.isImage ? (
                    <img
                      src={partner.logo}
                      alt={partner.name}
                      className="max-h-[26px] w-auto object-contain brightness-0 invert opacity-75"
                    />
                  ) : (
                    <span className="font-bold text-xs tracking-tight text-white/70 uppercase">
                      {partner.name}
                    </span>
                  )}
                </div>

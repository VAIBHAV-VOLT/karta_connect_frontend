  ...
  async function loadData() {
    ...
    try {
      ...
      if (comp) {
        ...
        // First get this company's posts

        // Get company's posts first

        const { data: companyPosts, error: postsError } = await supabase
          .from("job_posts")
          .select("*")
        ...
        const postIds = companyPosts.map((p) => p.id);
        ...
        if (postIds.length === 0) {
          ...
          const merged = apps.map((app) => ({
            ...app,
            student: students.find((s) => s.user_id === app.student_id) || null,

            job_post: companyPosts.find((p) => p.id === app.post_id) || null,
          }));
    ...
    return (
      <Card className="max-w-xl mx-auto mt-12">
        <CardHeader>
          ...
          <CardDescription>
            Setup your company name and settings before tracking candidates.
          </CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <CardContent className="p-6 space-y-4">
                <div className="flex flex-col md:flex-row justify-between items-start gap-4 pb-4 border-b">
                  <div className="flex gap-4">
                    ...
                    <div>
                      <h3 className="font-bold text-lg text-foreground">
                        {app.student?.name || "Karta Scholar"}
                      </h3>
                      <p className="text-sm font-semibold text-muted-foreground mt-0.5">
                        {app.student?.university} • {app.student?.course} (
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 shrink-0">
                    ...
                    <span
                      className={`text-[10px] border px-2 py-1 rounded-full uppercase font-bold tracking-wider ${statusStyles[app.status] || "bg-muted text-muted-foreground"}`}
                    >
                      {app.status}
                    </span>
                  </div>

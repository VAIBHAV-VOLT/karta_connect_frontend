...
function useTheme() {
    const [theme, setTheme] = useState(() => (typeof window !== "undefined" && localStorage.getItem("theme") === "dark" ? "dark" : "light"));
    useEffect(() => {
...
export function AppShell({ children }) {
    ...
    const isActive = (it) => {
        if (it.to !== path)
            return false;
        if (!it.search)
            return !search?.type || (it.to !== "/student/jobs" && it.to !== "/company/posts" && it.to !== "/admin/posts");
        ...
        <Sidebar collapsible="icon">
          ...
          <SidebarContent>
            <SidebarGroup>
              ...
              <SidebarGroupContent>
                <SidebarMenu>
                  {items.map((it) => (<SidebarMenuItem key={it.label}>
